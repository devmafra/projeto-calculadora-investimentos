...
export const createTable = (columnsArray, dataArray, tableId) => {
  if (
    ...
    throw new Error(
      "Para a correta execução, precisaos de um array com as colunas, outro comas as informações das linhas e também o id do elemento tabela selecionado"
    );
  }
...
function createTableHeader(tableReference, columnsArray) {
  ...
  if (tableHeaderReference.querySelectorAll("tr").length > 0) {
    return;
  }
...
function createTableBody(tableReference, tableItems, columnsArray) {
  ...
  for (const [itemIndex, tableItem] of tableItems.entries()) {
    ...
    for (const tableColum of columnsArray) {
      const formatFunction = tableColum.format ?? ((info) => info);
      tableRow.innerHTML += /*html*/ `<td class="text-center">${formatFunction(
        tableItem[tableColum.acessor]
      )}</td>`;
    }
